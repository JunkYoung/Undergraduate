	     ...
	     unsigned int retstack_size)/* num entries in ret-addr stack */
{
  ...
  case BPredComb:
    ...
    /* 2-level component */
    pred->dirpred.twolev = 
      bpred_dir_create(BPredGshare, 1, 1024*1024*1024, 29, 1);
  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPred2Level:
	...
	fatal("shift register width, `%d', must be non-zero and positive",
      ...
      pred_dir->config.two.shiftregs = calloc(l1size, sizeof(unsigned long long));
...
md_addr_t hash(md_addr_t baddr, unsigned int size) {
	baddr = (baddr >> 16) ^ (baddr >> MD_BR_SHIFT);
	unsigned int i = baddr;
	i = size*((i*((sqrt(5)-1))/2)-1);
	return (md_addr_t) i;
}


#define BIMOD_HASH(PRED, ADDR)					\
(hash(ADDR, (PRED)->config.bimod.size-1)) & ((PRED)->config.bimod.size-1)
	     ...
	     struct bpred_update_t *dir_update_ptr)/* pred state pointer */
{
  ...
  if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND) &&
      (pred->class == BPred2Level || pred->class == BPredGshare || pred->class == BPredComb))
    {
